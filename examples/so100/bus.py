"""Module for handling Feetech bus communication and joint control.

This module provides functionality for communicating with Feetech servos,
...
class FeetechBus:
    """A class for managing communication with Feetech servos.

    This class provides methods for reading and writing data to Feetech servos,
    including joint positions, velocities, and control parameters.
    """

    def __init__(self, port: str, description: dict[str, (np.uint8, str)]):
        """Initialize the FeetechBus.
    ...
    def close(self):
        """Close the connection to the Feetech bus."""
        self.port_handler.closePort()

    def write(self, data_name: str, data: pa.StructArray):
        """Write data to the Feetech servos.
    ...
    def read(self, data_name: str, motor_names: pa.Array) -> pa.StructArray:
        """Read data from the Feetech servos.
    ...
    def write_max_angle_limit(self, max_angle_limit: pa.StructArray):
        """Set the maximum angle limit for the servos.

        Args:
            max_angle_limit: A structured array containing the maximum angle limits.

        """
        self.write("Max_Angle_Limit", max_angle_limit)

    def write_min_angle_limit(self, min_angle_limit: pa.StructArray):
        """Set the minimum angle limit for the servos.

        Args:
            min_angle_limit: A structured array containing the minimum angle limits.
